...
Revision ID: 0d7b01602328
Revises: 
Create Date: 2026-05-01 17:03:02.464760
...
def upgrade() -> None:
    ...
    op.execute(
        """
        CREATE UNIQUE INDEX IF NOT EXISTS uq_active_folder_name_per_location
    ...
    op.execute(
        """
        CREATE UNIQUE INDEX IF NOT EXISTS uq_active_file_name_per_location
...
def downgrade() -> None:
    """Downgrade schema."""
    op.execute("DROP INDEX IF EXISTS uq_active_file_name_per_location")
    op.execute("DROP INDEX IF EXISTS uq_active_folder_name_per_location")
    op.execute("DROP INDEX IF EXISTS idx_files_owner_created_at")
    op.execute("DROP INDEX IF EXISTS idx_files_owner_folder")
    op.execute("DROP INDEX IF EXISTS idx_folders_owner_parent")
